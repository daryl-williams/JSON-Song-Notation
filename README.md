...
A song represented in JSN consists of a header and body which are both JSON objects. The header contains song metadata and the body contains the measures and beats of the song. The song's measures are represented as an array and each array element repersents a beat and each beat is an array containing some number of voices represented as JSON objects that occur on that beat.
